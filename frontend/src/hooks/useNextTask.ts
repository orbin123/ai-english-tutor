...
/**
 * Fetches the user's current task.
 *
 * Backend `/tasks/next` is idempotent: calling it repeatedly the same day
 * returns the same task. So it's safe to refetch / retry on focus.
 *
...
export function useNextTask(enabled = true) {
  return useQuery({
    ...
    enabled,
    staleTime: 5 * 60 * 1000, // 5 min — same task; no need to spam the server
